//add Websocket and Redis
const WebSocket = require("ws");
const redis = require("redis");
//create variables for the publisher and the client
let publisher;
let redisClient;
//constant for redis to expire after ten seconds
const redisExpireTimeInSeconds = 10;

let clients = [];

// Intiiate the websocket server over port 6379, 
const initializeWebsocketServer = async (server) => {
  ...
  // This is the subscriber part, creates a subscriber  and waits for it to be connected
  const subscriber = redisClient.duplicate();
  await subscriber.connect();
  // This is the publisher part, creates a publisher and waits for it to be connected
...
// If a new connection is established, the onConnection function is called, this happens when a message is being received
